...
description: AI 直接管理的 Typecho 博客知识库技能 — 创建、查询、更新、删除文章，内置分类标签体系、写作视角规范、脱敏规则与发布检查清单。当用户要求发布文章到博客、归档技术笔记、保存知识到 Typecho、查询或修改博客文章时触发此技能。涉及"发博客""写笔记""归档""知识库""Typecho"等关键词时均应使用。
version: 2.0.0
---
...
本技能需要用户已部署 Typecho 博客，并安装 OpenClawTypecho 插件 v1.1.0+。
...
6. **执行发布检查清单** — 逐项检查全部通过
7. **调用 API** — `action: submit`，`status: "publish"`
...
| `category` | string | ✅ | 分类名（白名单内） | — |
| `tags` | array | ✅ | 标签数组（3-5 个） | — |
| `slug` | string | 否 | URL 缩略名 | 自动生成 |
| `status` | string | 否 | 文章状态 | `publish` |
...
| 400 | 标题不能为空 | 检查 title 字段 |
| 400 | 正文不能为空 | 检查 text 字段 |
| 400 | 文章不存在 | get/update/delete 时 cid 无效 |
| 400 | 内容疑似包含敏感信息 | 正文检测到手机号/身份证/银行卡，检查并处理后重试 |
